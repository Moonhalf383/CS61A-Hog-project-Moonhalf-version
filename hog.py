...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def tamp_stratgy(score,opponent_score):
        return n
    return tamp_stratgy
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    tamp_roll = strategy(0,0)
    for i in range(0,goal):
        for j in range(0,goal):
            if strategy(i,j) != tamp_roll:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for i in range(1,11):
        tamp_dice = averaged_dice(i,dice)
        if tamp_dice > cnt:
            cnt = tamp_dice
            ans = i
    return ans
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    tamp_rate = 0
    tamp_index = 0
    for tamp_threshold in range(2,21):
        for tamp_roll in range(1,11):
            if average_win_rate(sus_strategy(score,opponent_score,tamp_threshold,tamp_roll)) > tamp_rate:
                tamp_rate = average_win_rate(sus_strategy(score,opponent_score,tamp_threshold,tamp_roll))
                tamp_index = sus_strategy(score,opponent_score,tamp_threshold,tamp_roll) 
    return tamp_index  # Remove this line once implemented.
    # END PROBLEM 12
